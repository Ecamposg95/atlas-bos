    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-2xl font-black text-dax-text">Pedidos Abiertos</h1>
        </div>
        <div className="flex items-center gap-2">
          <span className="text-dax-muted text-xs">{orders.length} pedidos</span>
      ...
      <DaxCard padding={false}>
        {loading ? <Spinner text="Cargando pedidos..." /> : orders.length === 0 ? (
          <div className="p-12 text-center">
            <i className="fa-solid fa-circle-check text-sem-success text-3xl mb-3 block" />
            <p className="text-dax-faint">Sin pedidos abiertos</p>
          </div>
        ) : (
              ...
              <tbody>
                {orders.map((s) => (
                  <tr key={s.id}>
                    ...
                    <td className="text-right font-semibold text-sem-success tabular-nums">{formatCurrency(s.total_amount)}</td>
                    <td><Badge variant="blue">{s.status}</Badge></td>
                    <td>
                      <button onClick={() => setSelected(s)} className="text-dax-muted hover:text-dax-text text-xs">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm" onClick={() => setSelected(null)}>
          <div className="dax-card p-6 w-full max-w-lg max-h-[80vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <div>
                <p className="text-[10px] text-dax-muted uppercase tracking-widest">Pedido</p>
            ...
            <div className="flex justify-between font-black text-dax-text border-t border-dax-border pt-3">
              <span>Total</span><span className="text-sem-success">{formatCurrency(selected.total_amount)}</span>
            </div>
          </div>
        </div>
      )}
