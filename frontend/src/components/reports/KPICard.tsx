...
export function KPICard({ label, value, previous, icon, tone = 'neutral', trend, format, noAnimate = false }: Props) {
  ...
  return (
    <div
      className={`group relative overflow-hidden rounded-xl border border-slate-700/40 ${styles.grad} p-3 transition-all duration-300 hover:-translate-y-0.5 hover:shadow-lg ${styles.border} ${styles.ring}`}
    ...
    >
      ...
      <div className="relative">
        ...
        {delta != null && (
          <p className={`mt-0.5 text-[10px] font-bold tabular-nums ${up ? 'text-emerald-400' : 'text-rose-400'}`}>
            <span aria-hidden>{up ? '▲' : '▼'}</span> {Math.abs(delta).toFixed(1)}%
            <span className="ml-1 text-[9px] font-medium text-slate-500">vs. período anterior</span>
          </p>
        )}
        {hasSpark && (
